...
def stockout_prevention(kpi_results: dict, inventory_data: pd.DataFrame) -> dict:
    """
    ...
    Args:
        kpi_results: Dictionary containing KPI results for each SKU, including
    ...
    for sku in kpi_results:
        ...
        if kpi_results[sku]["dos"] < 7:
            results["findings"].append({"sku_id": sku,
                                        "sku_name": sku_details["sku_name"].item(),
                                        "days_of_supply": kpi_results[sku]["dos"],
                                        "stockout_probability": kpi_results[sku]["sop"],
                                        "risk_level": "critical"
            })
            critical_count += 1
        elif kpi_results[sku]["dos"] < 14:
            results["findings"].append({"sku_id": sku,
                                        "sku_name": sku_details["sku_name"].item(),
                                        "days_of_supply": kpi_results[sku]["dos"],
                                        "stockout_probability": kpi_results[sku]["sop"],
        ...
        if kpi_results[sku]["sop"] > 0.7 or kpi_results[sku]["dos"] < 7:
            results["recommendations"].append({"sku_id": sku,
                                        "sku_name": sku_details["sku_name"].item(),
                                        "action": "reorder",
                                        "quantity": kpi_results[sku]["eoq"],
                                        "supplier": sku_details["supplier_id"].item(),
                                        "estimated_cost": kpi_results[sku]["eoq"] * sku_details["unit_cost"].item()
            })
